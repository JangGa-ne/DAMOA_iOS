...
// 교육뉴스
class SOSIK_EN: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        UIViewController.APPDELEGATE.SOSIK_EN_VC = self
    ...
    @objc func NETWORK_CHECK() {
        ...
        if SYSTEM_NETWORK_CHECKING() {
            GET_POST_DATA(NAME: "교육뉴스", BOARD_TYPE: "EN", LIMIT_START: 0)
        } else {
    ...
    func BEGINBATCHFETCH() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now(), execute: {
            ...
            if self.SYSTEM_NETWORK_CHECKING() {
                self.GET_POST_DATA(NAME: "교육뉴스", BOARD_TYPE: "EN", LIMIT_START: self.PAGE_NUMBER * self.ITEM_COUNT)
            } else {
...
extension SOSIK_EN: UITableViewDelegate, UITableViewDataSource {
