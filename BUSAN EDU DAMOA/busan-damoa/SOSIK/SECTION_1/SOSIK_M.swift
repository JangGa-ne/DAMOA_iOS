...
// 가정통신문
class SOSIK_M: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        UIViewController.APPDELEGATE.SOSIK_M_VC = self
    ...
    @objc func NETWORK_CHECK() {
        ...
        if SYSTEM_NETWORK_CHECKING() {
            GET_POST_DATA(NAME: "가정통신문", BOARD_TYPE: "M", LIMIT_START: 0)
        } else {
    ...
    func BEGINBATCHFETCH() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now(), execute: {
            ...
            if self.SYSTEM_NETWORK_CHECKING() {
                self.GET_POST_DATA(NAME: "가정통신문", BOARD_TYPE: "M", LIMIT_START: self.PAGE_NUMBER * self.ITEM_COUNT)
            } else {
...
extension SOSIK_M: UITableViewDelegate, UITableViewDataSource {
