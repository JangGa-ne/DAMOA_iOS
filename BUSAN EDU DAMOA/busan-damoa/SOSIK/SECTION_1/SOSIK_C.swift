...
// 학급알림장
class SOSIK_C: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        UIViewController.APPDELEGATE.SOSIK_C_VC = self
    ...
    @objc func NETWORK_CHECK() {
        ...
        if SYSTEM_NETWORK_CHECKING() {
            GET_POST_DATA(NAME: "학급알림장", BOARD_TYPE: "C", LIMIT_START: 0)
        } else {
    ...
    func GET_POST_DATA(NAME: String, BOARD_TYPE: String, LIMIT_START: Int) {
        ...
        for RECORD in UIViewController.APPDELEGATE.ENROLL_LIST { SC_CODE.append(RECORD.value(forKey: "sc_code") as? String ?? "")
            CL_CODE.append(RECORD.value(forKey: "cl_code") as? String ?? "")
        }
        for i in 0 ..< UNIQ(SOURCE: SC_CODE).count { if UNIQ(SOURCE: SC_CODE)[i] != "" { PARAMETERS["sc_code[\(i)]"] = UNIQ(SOURCE: SC_CODE)[i] } }
        for i in 0 ..< UNIQ(SOURCE: CL_CODE).count { if UNIQ(SOURCE: CL_CODE)[i] != "" { PARAMETERS["cl_code[\(i)]"] = UNIQ(SOURCE: CL_CODE)[i] } }
    ...
    func BEGINBATCHFETCH() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now(), execute: {
            ...
            if self.SYSTEM_NETWORK_CHECKING() {
                self.GET_POST_DATA(NAME: "학급알림장", BOARD_TYPE: "C", LIMIT_START: self.PAGE_NUMBER * self.ITEM_COUNT)
            } else {
...
extension SOSIK_C: UITableViewDelegate, UITableViewDataSource {
