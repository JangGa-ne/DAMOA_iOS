...
// 공지사항
class SOSIK_N: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        UIViewController.APPDELEGATE.SOSIK_N_VC = self
    ...
    @objc func NETWORK_CHECK() {
        ...
        if SYSTEM_NETWORK_CHECKING() {
            GET_POST_DATA(NAME: "공지사항", BOARD_TYPE: "N", LIMIT_START: 0)
        } else {
    ...
    func BEGINBATCHFETCH() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now(), execute: {
            ...
            if self.SYSTEM_NETWORK_CHECKING() {
                self.GET_POST_DATA(NAME: "공지사항", BOARD_TYPE: "N", LIMIT_START: self.PAGE_NUMBER * self.ITEM_COUNT)
            } else {
