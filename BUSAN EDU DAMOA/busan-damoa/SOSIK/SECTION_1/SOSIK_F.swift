...
// 오늘의식단
class SOSIK_F: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        UIViewController.APPDELEGATE.SOSIK_F_VC = self
    ...
    @objc func NETWORK_CHECK() {
        ...
        if SYSTEM_NETWORK_CHECKING() {
            GET_POST_DATA(NAME: "오늘의식단", BOARD_TYPE: "F", LIMIT_START: 0)
        } else {
    ...
    func BEGINBATCHFETCH() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now(), execute: {
            ...
            if self.SYSTEM_NETWORK_CHECKING() {
                self.GET_POST_DATA(NAME: "오늘의식단", BOARD_TYPE: "F", LIMIT_START: self.PAGE_NUMBER * self.ITEM_COUNT)
            } else {
...
extension SOSIK_F: UITableViewDelegate, UITableViewDataSource {
