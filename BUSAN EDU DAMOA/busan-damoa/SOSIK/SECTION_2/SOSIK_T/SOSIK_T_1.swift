...
// 학부모연수_디테일
class SOSIK_T_1: UIViewController {
    ...
    var SU_STAUT: String = "2"
    var IDX: String = ""
    
    var CLASS_TITLE: [String] = ["연수명", "연수일시", "장소", "대상", "모집인원", "연수소개", "신청기간", "상세정보", "담당부서", "담당자명", "문의처"]
    ...
    @IBAction func Submit_Button(_ sender: UIButton) {
        ...
        if SU_STAUT == "2" {
            if !SYSTEM_NETWORK_CHECKING() {
                ...
                HTTP_CHECK(IDX: IDX, TYPE: true)
            }
        } else {
            if !SYSTEM_NETWORK_CHECKING() {
                NOTIFICATION_VIEW("N: 네트워크 상태를 확인해 주세요")
            } else {
                HTTP_CHECK(IDX: IDX, TYPE: false)
            }
        }
    }
    ...
    @IBAction func Check_Button(_ sender: UIButton) {
...
//            print("[WEE센터]", response)
    ...
    @IBAction func Submit(_ sender: UIButton) {         // 수강신청
        ...
        if (Sc_name.text == "") || (CH_GRADE == "") || (CH_GENDER == "") || (Mb_name.text == "") || (Mb_password.text == "") {
            ...
            if !SYSTEM_NETWORK_CHECKING() {
                ...
                HTTP_INSERT(IDX: IDX, SC_NAME: Sc_name.text!, CH_GRADE: CH_GRADE, CH_GENDER: CH_GENDER, CH_PHONE: Mb_name.text!, CH_PASSWORD: Mb_password.text!, MODIFY: false)
            }
        }
    }
    ...
    func HTTP_CHECK(IDX: String, TYPE: Bool) {
        
        let INSERT_APPLY_STATUS = DATA_URL().SCHOOL_URL + "school/insert_apply_status.php"
        
        let PARAMETERS: Parameters = [
            "ch_phone": UIViewController.USER_DATA.string(forKey: "mb_id") ?? "",
            "ch_idx": IDX
        ]
        
        print("PARAMETERS -", PARAMETERS)
        
        Alamofire.request(INSERT_APPLY_STATUS, method: .post, parameters: PARAMETERS).responseJSON { response in
            
//            print("[수강신청확인]", response)
            
            if let CHECK_ARRAY = response.result.value as? [String: Any] {
                
                if CHECK_ARRAY["result"] as? String ?? "fail" == "success" {
                    
                    if TYPE == true {   // 수강신청
                        let VC = self.storyboard?.instantiateViewController(withIdentifier: "SOSIK_T_2") as! SOSIK_T_2
                        VC.modalTransitionStyle = .crossDissolve
                        VC.CLASS_CONTENT = self.CLASS_CONTENT
                        self.present(VC, animated: true, completion: nil)
                    } else {
                        self.NOTIFICATION_VIEW("이미 신청한 연수입니다")
                    }
                } else {
                    ...
                    if TYPE == false {  // 신청하기
                        ...
                        self.NOTIFICATION_VIEW("신청하지 않은 연수입니다")
                    }
                }
            } else {
    ...
    // 수강신청 등록
    func HTTP_INSERT(IDX: String, SC_NAME: String, CH_GRADE: String, CH_GENDER: String, CH_PHONE: String, CH_PASSWORD: String, MODIFY: Bool) {
        ...
        Alamofire.upload(multipartFormData: { multipartFormData in
            ...
            upload.responseJSON { response in
                ...
                if INSERT_DICT["result"] as? String ?? "fail" == "success" {
                    ...
                    let ALERT = UIAlertController(title: "신청되었습니다.", message: DATE_FORMATTER.string(from: Date()), preferredStyle: .alert)
...
extension SOSIK_T_1: UITableViewDelegate, UITableViewDataSource {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        
        if tableView == TableView {
            
            if CLASS_CONTENT.count == 0 {
